...
- 新增 **最后一次正确目标列表** 功能
- 1. set `last_worked_target_list=true` in `settings.conf`
- 1. 在 `settings.conf` 中设定 `last_worked_target_list=true`
- 2. Only available as `disable_module_as_brick=false` (the behavior logical is NOT compatible)
- 2. 仅当 `disable_module_as_brick=false` 时可用 (行为逻辑不兼容)
- 3. Bloatware Slayer will restore `target.conf` boot successfully at the last time
- 3. Bloatware Slayer 会恢复上次成功启动系统时的 `target.conf` 版本
...
- remove command from module files `su -c` forever
- 从模块文件中彻底移除 `su -c` 命令
- SHA256: `296ce32877538c19caaf42ba3d21be756f6316271326a64eadef570da1bd5906`
